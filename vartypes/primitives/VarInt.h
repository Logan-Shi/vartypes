...
namespace VarTypes {
  ...
  */
  ...
  class VarIntz;
  typedef shared_ptr<VarIntz> VarIntPtr;
  
  class VarIntz : public VarTypeTemplate<VarIntVal> 
  {
    Q_OBJECT
  protected:
  ...
  public:
    ...
    VarIntz(string name="", int default_val=0, int min_val=INT_MIN, int max_val=INT_MAX) : VarIntVal(default_val), VarTypeTemplate<VarIntVal>(name)
    {
      setMin(min_val);
      setMax(max_val);
